...
PROMPT_TEMPLATE = """Ты — редактор Telegram-канала о кино на русском языке.
...
Новость с сайта {source}:
Заголовок: {title}
Текст: {summary}

Верни JSON строго в таком формате:
{
  "ru_title": "Заголовок на русском (до 80 знаков)",
  "ru_text": "Текст поста на русском (до 900 знаков)"
}"""


def call_gemini(api_key: str, prompt: str) -> Optional[str]:
    """Вызываем Gemini API через стандартный urllib (без сторонних библиотек)."""
